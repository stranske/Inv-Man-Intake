...
- [x] Add deterministic firm/fund name normalization before intake IDs are minted.
- [x] Reuse existing firm/fund IDs for normalized name variants, while preserving explicit IDs.
- [x] Persist observed firm aliases in `aliases_json` and add repository list helpers for deterministic identity lookup.
- [x] Add identity-resolution tests covering LLC/LP variants, negative-control distinct firms, and existing acceptance smoke behavior.

### Acceptance Criteria
The PR is complete when ALL of these are satisfied:

- [x] `python -m pytest tests/intake/test_ingest_core_registry.py tests/intake/test_ingest_integration.py tests/intake/test_identity_resolution.py --no-cov`
- [x] `python -m pytest tests/intake/test_identity_resolution.py tests/test_v1_acceptance_smoke.py tests/data/test_repository_core.py -q --no-cov`
- [x] `python -m ruff check src/inv_man_intake/intake/integration.py src/inv_man_intake/data/repository.py tests/intake/test_identity_resolution.py`
- [x] `python -m ruff format --check src/inv_man_intake/intake/integration.py src/inv_man_intake/data/repository.py tests/intake/test_identity_resolution.py`
- [x] `python -m mypy src/inv_man_intake/intake/integration.py src/inv_man_intake/data/repository.py`

### Recently Attempted Tasks
Avoid repeating these unless a task needs explicit follow-up:

- Add deterministic firm/fund name normalization before intake IDs are minted.

### Suggested Next Task
- Reuse existing firm/fund IDs for normalized name variants, while preserving explicit IDs.
