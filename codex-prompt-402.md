...
### Scope
The v1 plan names eight first-wave asset classes: equity market neutral, quant, multi-strat, credit long/short, macro, trend following, credit relative value, and activist. Current scoring configuration and fixtures use broader classes such as `equity`, `credit`, `macro`, `multi_strategy`, and `real_assets`. That mismatch makes it unclear whether the scoring engine satisfies the design requirement for asset-class-specific scoring.

### Tasks
Complete these in order. Mark checkbox done ONLY after implementation is verified:

- [x] Decide and document the canonical v1 asset-class keys for the eight classes in `docs/INV_MAN_INTAKE_PLAN_V1.md`.
- [x] Update `src/inv_man_intake/scoring/weights.py` and `config/scoring_weights/` to cover the canonical v1 classes.
- [x] Add alias handling for existing fixture values such as `equity`, `credit`, `multi_asset`, or `long_short_equity` when they should map to a canonical v1 class.
- [x] Update scoring regression fixtures and tests to cover all launch classes.
- [x] Add validation that rejects unmapped asset-class labels with a clear error message.

### Acceptance Criteria
The PR is complete when ALL of these are satisfied:

- [x] `python -m pytest tests/scoring --no-cov` passes.
- [x] `load_weight_registry()` returns coverage for every approved v1 launch asset class.
- [x] Scoring tests include at least one fixture for each approved v1 launch class.
- [x] The docs list the canonical keys and any accepted aliases.
- [x] Unknown or unsupported asset-class labels fail with a deterministic validation error.

### Recently Attempted Tasks
Avoid repeating these unless a task needs explicit follow-up:

- Decide and document the canonical v1 asset-class keys for the eight classes in `docs/INV_MAN_INTAKE_PLAN_V1.md`.
- checkbox-progress
- no-focus
