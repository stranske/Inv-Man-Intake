...
|---|---|---|---|---|
| IMI-1 | an analyst can ingest a document package and sees a registered run with document IDs and artifacts | `inv-man-ingest <bundle.json> --out <dir>` | ingest Alpha vs Beta bundles with co-located PDF/XLSX; compare run IDs | WORKS |
...
| IMI-6 | an analyst can review validation queue and sees filterable paginated rows | `validation_queue_api.list_validation_queue` | filter queue rows by `package_id`; inspect CLI-to-queue linkage | PARTIAL |
