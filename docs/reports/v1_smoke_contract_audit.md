...
| `core_schema.versioned_documents` | `docs/contracts/core_schema.md` | end-to-end | `tests/test_v1_acceptance_smoke.py:54-56` asserts stable document metadata after registration; the smoke path uses repository-backed document records rather than dict-only fixtures. | Covered by #379 / PR #393. |
| `core_schema_migration.apply_rollback` | `docs/contracts/core_schema_migration.md` | outside v1 smoke | `src/inv_man_intake/v1_smoke.py` uses an in-memory repository for the acceptance path, and `tests/test_v1_acceptance_smoke.py` has no migration apply/rollback assertion. | No issue filed from this audit; migration runners are outside the v1 smoke claim. |
...
| `queue_states.orphan_state_machine_absence` | `docs/contracts/queue_states.md` | regression guard | `tests/v1/test_smoke_contract_coverage.py` rejects imports from `inv_man_intake.queue.state_machine`. | Covered by #381 / PR #400. |
...
| `queue_assignment_sla.ops_reassignment` | `docs/contracts/queue_assignment_sla.md` | outside v1 smoke | The v1 smoke path exercises analyst-first assignment for a validation item, not ops reassignment flows. | No issue filed from this audit; ops reassignment is outside the v1 smoke claim. |
| `queue_assignment_sla.sla_breach_scheduling` | `docs/contracts/queue_assignment_sla.md` | outside v1 smoke | The v1 smoke path does not schedule or age SLA breach jobs. | No issue filed from this audit; SLA scheduling is outside the v1 smoke claim. |
...
| `provenance_history.source_location_trace` | `docs/contracts/provenance_history.md` | end-to-end boundary | `tests/test_v1_acceptance_smoke.py:60-75` asserts source document and source page from extraction into the smoke result. | No issue filed; source-location trace is covered by #380 / PR #399. |
| `provenance_history.field_corrections` | `docs/contracts/provenance_history.md` | outside v1 smoke | The v1 smoke does not write append-only correction history records. | No issue filed from this audit; correction-history behavior is outside the v1 smoke claim. |
| `agent-runner-output.workflow_call_outputs` | `docs/contracts/agent-runner-output.md` | outside v1 smoke | No call site exists in `src/inv_man_intake/v1_smoke.py`; this contract targets automation runner output artifacts rather than app smoke behavior. | No issue filed from this audit; runner output artifacts are outside the v1 smoke claim. |
| `smoke_trace.continuity` | v1 smoke acceptance path | end-to-end | `tests/test_v1_acceptance_smoke.py:111-116` asserts intake, extraction, normalization, queue, and scoring trace markers. | No issue filed; direct smoke evidence exists. |
