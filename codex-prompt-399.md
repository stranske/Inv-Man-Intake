...
**Progress:** 8/8 tasks complete, 0 remaining
...
- [x] Add a dependency-free PDF byte provider that validates PDF framing, extracts literal stream text, and emits canonical fields with page provenance.
- [x] Route the v1 smoke primary extraction through ExtractionOrchestrator using committed PDF bytes instead of hard-coded fixture fields.
- [x] Route secondary XLSX bytes through the same provider boundary with deterministic unsupported-format escalation.

### Acceptance Criteria
The PR is complete when ALL of these are satisfied:

- [x] python -m ruff check src/inv_man_intake/extraction/providers/pdf_primary.py src/inv_man_intake/extraction/providers/__init__.py src/inv_man_intake/v1_smoke.py tests/extraction/test_pdf_primary_provider.py tests/extraction/test_document_byte_provider.py tests/test_v1_acceptance_smoke.py
- [x] python -m mypy src/inv_man_intake/extraction/providers/pdf_primary.py src/inv_man_intake/v1_smoke.py
- [x] python -m pytest tests/extraction/test_pdf_primary_provider.py tests/extraction/test_document_byte_provider.py --no-cov
- [x] python -m pytest tests/extraction tests/test_v1_acceptance_smoke.py --no-cov
- [x] rg "fixture-primary|primary_extractor" src/inv_man_intake/v1_smoke.py tests/test_v1_acceptance_smoke.py

### Recently Attempted Tasks
Avoid repeating these unless a task needs explicit follow-up:

- Add a dependency-free PDF byte provider that validates PDF framing, extracts literal stream text, and emits canonical fields with page provenance.

### Suggested Next Task
- Route the v1 smoke primary extraction through ExtractionOrchestrator using committed PDF bytes instead of hard-coded fixture fields.
