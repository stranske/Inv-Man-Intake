...
_DDQ_ELEMENTS = (
    ...
    StandardElement(key="inception_date", detector_name="fundamentals.inception_date", mandatory=False),
    StandardElement(key="management_fee", detector_name="terms.management_fee", mandatory=False),
    StandardElement(key="investment_minimum", detector_name="terms.investment_minimum", mandatory=True),
    StandardElement(key="lockup_period", detector_name="terms.lockup_period", mandatory=False),
    StandardElement(key="redemption_frequency", detector_name="terms.redemption_frequency", mandatory=False),
...
def test_ddq_fields_extracted() -> None:
    ...
    try:
        ...
        coverage = _DDQ_LIBRARY.evaluate_coverage("ddq", {k: v for k, v in complete_ddq_fields.items()})
    ...
    try:
        ...
        coverage_incomplete = _DDQ_LIBRARY.evaluate_coverage(
            "ddq", 
            {k: v for k, v in incomplete_ddq_fields.items()}
        )
        
        # Find the investment_minimum coverage
        investment_min_coverage = next(
            (c for c in coverage_incomplete if c.key == "investment_minimum"),
            None
        )
    ...
    try:
        ...
        coverage_restored = _DDQ_LIBRARY.evaluate_coverage(
            "ddq",
            {k: v for k, v in restored_ddq_fields.items()}
        )
        
        investment_min_coverage_restored = next(
            (c for c in coverage_restored if c.key == "investment_minimum"),
            None
        )
...
def test_ddq_required_fields_contract() -> None:
    ...
    mandatory_ddq_fields = {
        elem.key for elem in _DDQ_ELEMENTS if elem.mandatory
    }
    
    expected_mandatory = {"firm_name", "fund_name", "strategy", "aum", "investment_minimum"}
    
    assert mandatory_ddq_fields == expected_mandatory, \
...
def test_ddq_document_type_classification() -> None:
    ...
    for text in ddq_strings:
        ...
        assert doc_type == DocumentType.DDQ, \
